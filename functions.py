...
# - - - FUNKCJA TRENUJACA - - -
def test_network(net, test_data, types_number):
    predicted_labels = []
    true_labels = []
    output_array = []
    global_error = 0
...
def calculate_to_logs(y_test, y_pred, types_list):
    correct = 0
    types_list_1 = prepare_data_012(types_list)
    individual_correct_list = [0] * len(types_list_1)
    ...
    confusion_matrix = [[0 for _ in range(len(types_list_1))] for i in range(len(types_list_1))]
    for t, r in zip(y_test, y_pred):
        confusion_matrix[t][r] += 1
    precision = [0] * len(types_list_1)
    recall = [0] * len(types_list_1)
    f_measure = [0] * len(types_list_1)

    for i in range(len(types_list_1)):
        tp = confusion_matrix[i][i]
        fp = sum(confusion_matrix[j][i] for j in range(len(types_list_1))) - tp
        fn = sum(confusion_matrix[i][j] for j in range(len(types_list_1))) - tp
